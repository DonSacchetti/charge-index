...
- **Vercel**: project `charge-index` under team `soenen-strategies`, on the **Hobby** plan. Deployed via `vercel deploy --prod` using a token scoped to Jen's account, stored in `../.env.local`. **Deploys are currently blocked** (since 2026-09-14): Hobby only lets the account owner deploy, Vercel checks the git commit author, and our commits are authored by Josh's git identity. The deployment shows `BLOCKED` with "the commit author doesn't have permission" and the CLI hangs waiting on it. Hobby is also non-commercial use only per Vercel's own docs, which this product isn't. The fix is Vercel Pro on Jen's account with Josh as a Developer seat — Jen's decision. Do not work around the block by stripping git metadata or authoring commits as Jen. GitHub → Vercel continuous deployment is also not connected.
...
Phase 4 (daily check-in grid) and Phase 5 (weekly map engine + client results) are built, verified and pushed, but **not deployed** — production is still on Phase 3 because of the Vercel block above. Phase 2 (payments) stays deferred until Jen creates the Stripe account. Next: Phase 6, the coach view.
